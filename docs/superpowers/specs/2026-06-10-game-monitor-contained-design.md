...
3. **Wasted re-fetching, no lookahead.** Each run fetches its witness data from cold when the game is already due (`cost_estimator.rs:297-305`); a retry re-fetches everything. Nothing is generated ahead of time, even though the L2 blocks for a range are produced long before the game must be cost-estimated.
...
- **Predictive pre-generating.** Don't wait for a game to be due. Predict each range from the proposal cadence and **generate its SP1 stdin as soon as its L2 blocks finalize**, so a game's witness is ready before it's proved.
- **Fetch/crunch once, cache both.** Cache the `WitnessData` (output of `host.run`) and the `SP1Stdin` (output of `get_sp1_stdin`) on disk, so a rerun never re-fetches/re-derives/re-proves and a prove never re-runs.
...
| D2 | **Fully in-process** (one OS process) | User requirement. Consequence: no SIGKILL isolation, and memory must budget **both** witness-gen and prove — see §7. |
| D3 | **Predictive, finalization-triggered witness pipeline** (single design, no phases) | Predict ranges from `PROPOSAL_INTERVAL`; generate `WitnessData`+`SP1Stdin` as each range's end finalizes, ahead of proving — see §4.2. |
| D4 | **Cache both `WitnessData` and `SP1Stdin`** on disk (plain blobs, no DB), pruned on separate clocks | `WitnessData` is the fetch-expensive durable artifact; `SP1Stdin` is what prove consumes — see §4.4. |
...
Two new artifacts in the op-succinct repo. The daemon has two cooperating concerns — a **proactive witness pipeline** (producer) and a **reactive game prover** (consumer) — that hand off through the on-disk cache (§4.4).

### 4.1 `scripts/utils/bin/game_monitor_contained.rs` — the daemon

**Reactive control plane (the game prover).** Keeps today's proven semantics:
...
- **Discover→prove `--delay`** — the window that avoids the multi-backend RPC 404 race (`:130-136`). Preserved.
- **Proving** — for each of a game's safe-head sub-ranges, **load the pre-generated `SP1Stdin` from the cache** (a hit, because the pipeline generated it ahead of time; on a miss, generate it on demand) and run the SP1 execute (the ~50 GB step), gated by RSS admission (§7). Aggregate `ExecutionStats`.
...
A background pipeline that generates SP1 stdin **ahead** of proving, so a game's witness is ready before it's due:

1. **Predict the window.** Game boundaries step by `PROPOSAL_INTERVAL` (`celo/create_new_game_branch.sh:83`: `l2 = parent + PROPOSAL_INTERVAL`). The pipeline tracks the frontier (last generated end) and the next window is `[frontier, frontier + PROPOSAL_INTERVAL]`. Configure the monitor with the proposer's interval. (Best-effort/speculative: the on-chain game is the source of truth, so a wrong interval only wastes work and the prover regenerates on demand.)
...
4. **Bounded concurrency.** Multiple sub-range witness units run concurrently, gated by the same RSS budget as proving (witness-gen has its own multi-GB footprint — §7).

When the prover later processes a game, every sub-range's stdin is already cached → it just proves. Because the pipeline (over a predicted window) and the prover (over the real game `[start, end]`) run the **same deterministic safe-head split**, the keys match whenever the prediction matched the real game boundary.
...
// reactive: load stdin (generate the witness on miss) + run the SP1 prove. Called by the prover (§4.1).
pub async fn prove_range(
    cache, range: SpanBatchRange, opts,
) -> Result<ExecutionStats, EstimatorError>;
```

- `witness_range` runs `host.run` then `get_sp1_stdin` **sequentially** (stdin depends on `WitnessData`), caching each.
- `prove_range` loads the cached stdin and runs SP1 inside its own `tokio::task::spawn_blocking` (`CpuProver` spins its own tokio runtime and must not run on the async runtime, `cost_estimator.rs:132-135`). **Concurrency across ranges is governed by the daemon's RSS admission gate (§7), not an uncontrolled rayon `par_iter` over all ranges** (`:136`) — that would bypass admission.
...
- **Per-sub-range witness retry:** wrap `host.run → get_sp1_stdin`; on a *successful* `host.run` the `WitnessData` is cached (§4.4), so a later retry skips it. Once stdin is cached, a later **prove** failure costs only a re-prove — the key resilience win (no hour of re-fetch/re-derive/re-prove).
...
`tracing` subscriber emitting **JSON to stdout** (collected by infra). One span per unit of work carrying `game` + `range` + `attempt`; `host.run`/`get_sp1_stdin`/`prove` as child spans. Enable the `log` bridge so existing `log::`-based library code is captured without a big-bang migration. Replaces fs-`metadata().len()` health polling and the 1 MiB log-tail scrape.

## 5. Data flow

**Producer — the witness pipeline (proactive, continuous):**
```
track frontier = last generated end
predict next window [frontier, frontier + PROPOSAL_INTERVAL]
  ...
  → for each sub-range, when its end block is L2-finalized (and L1 finalized past l1_head):
        ...
        drop cached WitnessData (stdin now generated)
  → advance frontier
```

**Consumer — the game prover (reactive):**
```
discover index (finalized) → wait --delay → fetch game data (type-42; real start/end)
  → safe-head split into sub-ranges
  → for each sub-range:
        SP1Stdin cached?  ── hit ──► prove (RSS-admitted) → ExecutionStats
                          ── miss ─► witness_range (on demand) → prove
  → aggregate ExecutionStats → mark game success → schedule stdin prune (after grace)
```

**Failure cascade (innermost first):** per-call retry (transient RPC) → per-sub-range witness retry (a successful `host.run` is cached, so a later prove failure skips it) → per-game requeue (Primary → Background; `last_contiguous` still advances so the daemon never stalls).

## 6. Code-sharing seams (what to lift / change)

- **Lift** `execute_blocks_and_write_stats_csv` + `aggregate_execution_stats` (`cost_estimator.rs:35-251`) into `utils/estimator`, split into `witness_range` (fetch+crunch+cache) and `prove_range` (load+prove); return `ExecutionStats` (drop CSV write/reread + `cargo_metadata` workspace_root lookup).
...
- **Two heavy workloads share the budget.** Witness-gen (`host.run`, multi-GB of preimages held in memory) now runs **concurrently** with SP1 execute (~50 GB). RSS admission must gate **both** kinds of work — pipeline witness units and prover proves draw from the same budget.
- **Per-prove bound:** `--batch-size` caps L2 blocks per range → caps SP1 guest memory per prove. Default **conservatively** so any single prove is a fraction of the pod.
- **Adaptive concurrency:** extend the completion-history (`completion_history.json`) to record **peak RSS** for both witness and prove, keyed by EVM gas (tracks memory better than block count). Project the next unit's peak from history; admit only if it fits remaining budget with a margin.
...
| Late mid-pipeline L1/beacon fetch | `host.run` via kona `OnlineHostBackend`; `OnlineBlobStore` | none; aborts after ~1 h | inline `RetryBackoffLayer`; on `host.run` success cache `WitnessData` so a later (prove) failure skips it |
...
**Retry boundaries (smallest re-doable unit first):** per-RPC-call → per-sub-range `host.run` (on success cached as `WitnessData`) → per-range prove (don't retry OOM) → per-game (`maybe_requeue`, rarely fires).

## 9. Preserved vs changed

- **Preserved:** finalized index polling, type-42 filter, discover→prove `--delay`, `last_contiguous` semantics, `progress.json` resume, Primary/Background retry policy.
- **Changed:** subprocess + log-scrape → in-process structured `Result`; **cold, on-demand witness-gen → predictive pipeline that pre-generates stdin as blocks finalize**; SIGKILL supervision → cooperative cancellation + RSS admission covering witness-gen *and* prove; `log` → `tracing` spans to stdout; CSV/log IPC removed.
...
- **Integration:** run a small real L2 range in-process and assert aggregated stats match a `cost_estimator` baseline; assert the pipeline pre-generates a range's stdin once its end finalizes and the prover then **skips `host.run`** (cache hit); inject a forced mid-witness RPC failure and assert recovery without redoing a cached step; assert stdin prune evicts after grace and `WitnessData` is dropped once stdin is generated.

## 11. Risks & mitigations

- **Dual memory pressure (D2):** witness-gen and prove now share one address space with no SIGKILL. → conservative batch-size, RSS admission covering both, harden the `Mutex`/KZG `.unwrap()`s in `online_blob_store.rs`.
- **No cooperative cancellation of prove:** a runaway pure-CPU prove can't be stopped. → batch-size bounds runtime; network hangs handled by timeouts; absolute-duration becomes warn+alert.
- **Prediction accuracy:** a wrong `PROPOSAL_INTERVAL` (config drift, off-cadence game) pre-generates ranges that never hit. → best-effort only (on-chain game is source of truth); prover regenerates on miss; cap pipeline lead distance; prune speculative misses by age.
...
- Optionally prove speculatively (not just generate the witness) for predicted games, if lookahead on the ~50 GB step is worth the speculative memory.
